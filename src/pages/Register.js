...
function Register() {
  const [formData, setFormData] = useState({
    name: "",
    occupation: "",
    participatedIn: [], // This will store multiple selected items
    reasonsForInvoiceDiscounting: [], // This will store multiple selected reasons
    referralCode: "",
  });
  const [isLoading, setIsLoading] = useState(false);
  const [message, setMessage] = useState("");
  ...
  const handleChange = (e) => {
    const { name, value, type, checked } = e.target;
    if (type === "checkbox") {
      // Update the respective array when a checkbox is checked/unchecked
      setFormData((prevData) => ({
        ...prevData,
        [name]: checked
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (response.data.token) {
        ...
        // Navigate to the dashboard after successful profile completion
        navigate("/c-dashboard"); // You can conditionally use different paths or additional states
      }
    } catch (error) {
      console.error("Error completing profile:", error);
      setMessage("Error completing profile. Please try again.");
